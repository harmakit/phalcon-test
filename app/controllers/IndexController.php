...
class IndexController extends ControllerBase
{
    ...
    public function tableAction(int $id = 1)
    {
        $start = microtime(true);
        ...
        $tempTableName = 'selected_people_' . $id . '_' . sha1(microtime());
        $this->db->query('
            CREATE TABLE IF NOT EXISTS ' . $tempTableName . '
              AS (
                    SELECT p.* FROM people p
                    WHERE p.region_id IN ' . $childrenIdentifiers . '
                 )
        ')->execute();
        ...
        for ($i = 0; $i < 3; $i++) {
            ...
            $randomPeopleIdentifiers = [];
            foreach ($randomPeople as $randomPerson) {
                $randomPeopleIdentifiers [] = $randomPerson['id'];
            }
            $randomPeopleIdentifiers = '(' . implode(',', $randomPeopleIdentifiers) . ')';

            while (!$row) {
                $inClause = (count($randomPeople) !== 0) ? 'AND p.id NOT IN ' . $randomPeopleIdentifiers : '';

                $row = $this->db->query('
                    SELECT p.* FROM ' . $tempTableName . ' p
                    JOIN ( SELECT ' . $rand . ' * (SELECT MAX(id) FROM ' . $tempTableName . ') AS max_id ) AS m
                    WHERE p.id >= m.max_id '
                    . $inClause .
                    'ORDER BY p.id ASC
                    LIMIT 1;
